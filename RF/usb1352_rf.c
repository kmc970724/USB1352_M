...
//    uint32_t start_time, end_time;
    ...
    while (1)
    {
        usb1352_rf_transfer(&u_frame, U_FRAME_BEACON, 0x01);
        usb1352_rf_receive(&u_frame, sizeof(sub1ghz_u_frame), DATA_SLOT_SIZE);
        if (!(usb1352_check_frame(&u_frame, U_FRAME_UACK)))
        {
            continue;
        }

        u_frame.fc.u_code_2 = tx_seq;
        usb1352_rf_transfer(&u_frame, U_FRAME_POLL_RR, 0x01);
        usb1352_rf_receive(&s_frame, sizeof(sub1ghz_s_frame), DATA_SLOT_SIZE);
        if (!(usb1352_check_frame(&s_frame, S_FRAME_RR)))
        {
            continue;
        }
        rx_seq = s_frame.fc.rx_seq;
...
//        end_time = Clock_convertMsToTicks(SUPER_FRAME_SIZE) + start_time;
        ...
        while (1)
        {
            if (!(queue_empty(my_dev->rf_data_tx_queue)))
            {
                ...
                Task_sleep(Clock_convertUsToTicks(500));
